...
int main(int argc,char** argv)
{
    ...
    int times = 1;
    if(JudgeEpoll(listenfd))//判断监听套接字是否可读
    {
        printf("times: %d\n",times);
        ++times;
        ...
        if(pthread_create(&pid,NULL,ReadImgMessage,NULL) < 0)
        {//启动图片读取线程
            perror("pthraed_create error\n");
            exit(0);
        }

        if(pthread_create(&tid,NULL,SendImg,(void*)&connetfd) < 0)
        {//启动发送文件线程
            perror("pthread_create error\n");
            exit(0);
        }
        printf("/********************************************************/\n");
        pthread_join(pid,NULL);//捕捉文件读取线程结束信号          
        printf("/********************************************************/\n");
    }
    sleep(1);
    close(connetfd);
    ...
    while((dirp=readdir(dp)) != NULL)//循环读取文件中所有的图片
    {
        ...
        {//然后等待下一次tail和head指向同一个节点
            if(Deque.tail == Deque.head)
            {
                ...
                memset(Deque.tail->poet.filename,'\0',SIZE);
                strcpy(Deque.tail->poet.filename,dirp->d_name);
                Deque.tail->poet.filesize = sta.st_size;

                memset(Deque.tail->poet.data,'\0',DATASIZE);
                times = Readn(fd,Deque.tail->poet.data,DATASIZE);
...
static void* SendImg(void* arg)
{
    ...
    while(1)//如果head->next指向tail，发送Head的节点，然后将head移动到tail指向的节点上
    {
        if(Deque.head->next == Deque.tail)
        {
            ...
            if(send(connetfd,&st->poet,PACKETSIZE,0) < 0)
            {
                perror("send error!\n");
                exit(0);
            }
